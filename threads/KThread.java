...
public class KThread {
	...
	public static void finish() {
		...
		// If this thread is child of a parent
		if (currentThread.parentThread != null) {
			// Machine.interrupt().disable();
			// System.out.println(currentThread.getName() + " just finished");
			// System.out.println(currentThread.parentThread.getName() + " just readied");
	...
	public void join() {
		...
		// Lib.assertTrue(this.parentThread != null);

		
		// // case: this thread is not running
		if (this.status == statusFinished) {
			return;
		}
		// case: this thread is not finished running
		this.joined = true;
	...
	private static void joinTest2() {
		...
		KThread childA = new KThread( new Runnable () {
			public void run() {
				System.out.println("Child A has started");
				for (int i = 0; i < 10; i++) {
					System.out.println ("Child A: busy...");
					// KThread.currentThread().yield();
				}
				System.out.println("Child A has ended");
			}
		});
		KThread childB = new KThread( new Runnable () {
			public void run() {
				System.out.println("Child B has started");
				for (int i = 0; i < 10; i++) {
					System.out.println("Child B: busy...");
				}
				System.out.println("Child B has ended");
			}
		});
		KThread childC = new KThread( new Runnable () {
			public void run() {
				System.out.println("Child C has started");
				childB.join();
				childA.join();
				for (int i = 0; i < 30; i++) {
					System.out.println("Child C: busy...");
				}
				System.out.println("Child C has ended");
			}
		});
		...
		System.out.println("Testcode Started");
		for (int i = 0; i < 20; i++) {
			System.out.println ("Testcode: busy...");
			KThread.currentThread().yield();
		}
	}
